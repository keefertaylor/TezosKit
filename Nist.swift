...
//      SecKeyAlgorithm.ecdsaSignatureMessageX962SHA256,
    ...
    @available(OSX, unavailable)
    @available(iOS, deprecated: 10.0, message: "This method and extra complexity will be removed when 9.0 is obsolete.")
    public static func signUsingSha256(_ digest: Data, privateKey: PrivateKey) throws -> Data {
      #if os(iOS)
      Helper.logToConsoleIfExecutingOnMainThread()
//      let digestToSign = digest.

      var digestToSignBytes = [UInt8](repeating: 0, count: digest.count)
      digest.copyBytes(to: &digestToSignBytes, count: digest.count)

      var signatureLength = 74
